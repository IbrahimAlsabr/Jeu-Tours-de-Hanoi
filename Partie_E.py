def afficher_meilleurs_scores(dict_des_resultats):
    '''
    enregistrer le score du joueur (nom du joueur, nombre de disques, nombre de coups) dans un ´
    fichier de scores. On pourra utiliser le module time pour aussi enregistrer la date de la partie.
    '''
...
def temps_moyen(dict_des_resultats):
    ...
    for key, value in dict_des_resultats.items():
        for cles, valeurs in value.items():
            if (cles == "time"):
                dic_temps_moyen[key] = valeurs / \
                    dict_des_resultats[key]['nb_coups']
